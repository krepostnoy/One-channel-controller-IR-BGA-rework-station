//Крайнее редактирование 27.03.2022. Эта версия для http://radioservice.at.ua/forum/23-27-1
//---ProMini_LCD1602_ANALOG KEYBOARD SHILD---//
#include <EEPROM.h>
#include "Setting.h"
#include "Variables.h"
...
void loop() {                                                   //основной цикл программы
  ...
  switch (reflowState) {                                        //машина состояний (swich)
    case REFLOW_STATE_IDLE: {                                   //главный экран
        ...
        digitalWrite(BH_OFF, LOW);                             //выключить НИ
...
void PushOk() {                                                 //обработка короткого нажатия Ok
  if (reflowState == REFLOW_STATE_IDLE) {
    ...
    digitalWrite(Int_Fan, HIGH);                                //включить охладитель симисторов и контроллера
    digitalWrite(BH_OFF, HIGH);                                 //включить НИ
    integra = 0;
    p = 0;
    d = 0;
    Count1 = 0;
...
void PushOkLong() {                                             //обработка длинного нажатия Ok
  if (reflowState == REFLOW_STATE_PREHEAT || reflowState == REFLOW_STATE_STEP_RAMP ||
      reflowState == REFLOW_STATE_STEP || reflowState == REFLOW_STATE_STEP_DWELL ) { //ничего не делаем в этих состояниях
    return;
  }
...
}                                                               //конец обработки нажатия ВПРАВО                                                                               //закрываем PushRight

void PushLeft() {                                               //обработка короткого нажатия Left
  if (reflowState == REFLOW_STATE_IDLE) {
    digitalWrite(Ext_Fan, LOW);
    return;
  }
  if (reflowState == REFLOW_STATE_PREHEAT || reflowState == REFLOW_STATE_STEP_RAMP
      || reflowState == REFLOW_STATE_STEP || reflowState == REFLOW_STATE_STEP_DWELL) {
    reflowState = REFLOW_STATE_COMPLETE;
    reflowStatus = REFLOW_STATUS_OFF;
    return;
  }
