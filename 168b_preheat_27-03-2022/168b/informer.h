...
/* Крайнее редактирование 27.03.2022. Эта версия для http://radioservice.at.ua/forum/23-27-1
  ...
  комплектующих: плата arduino на основе микроконтроллеров AtMega168p / 328p, (5v / 16MHz) с
  доступными контактами A6 (к нему подключается клавиатура) и A7; LCD1602a без i2c;
  один модуль MAX6675; самодельная (или покупная) аналоговая (резистивная) клавиатура на 5 кнопок;
  самодельная (или покупная) 3х- или 4х-канальная плата реле на 12в (не обязательно);
  одно твердотельное реле типа SSR-40DA - SSR-15DA, или его самодельный аналог; 2 вентилятора на 12в (не обязательно);
  ...
  имеет широкий набор функций: позволяет управлять включением/отключением
  вентилятора охлаждения платы после завершения пайки; автоматически завершает
  пайку при потере контакта с термодатчиком; позволяет строить график
  ...
  неё пином А0 (выход на реле управления ON/OFF термостола), активный уровень высокий (HIGH).
  2. Излучатель (нагреватель) включится после короткого нажатия кнопки ВПРАВО (RIGHT) во время пайки,
  ...
  3. После окончания, или остановки вручную, профиля, автоматически НЕ ВКЛЮЧИТСЯ охладитель
  платы пином А5 (выход на реле управления вентиляторами охлаждения платы после
  завершения пайки), активный уровень высокий (HIGH).
  4. Выключить охладитель платы можно из режима IDLE клавишей ВЛЕВО (LEFT).
  ...
  и охладитель используется как вентилятор :) ).
  ...
  17. Короткое нажатие кнопки OK - запуск пайки. Удержание 5+ секунд кнопки OK в IDLE - вход
  в меню. Если находимся в меню, то долгое нажатие ОК сохранит сделанные в меню
  изменения и выведет контроллер в состояние IDLE.
  18. Короткое нажатие кнопки ВЛЕВО (LEFT) в меню и в режиме пайки, переводит контроллер в состояние IDLE.
  19. Удержание 5+ секунд кнопки ВЛЕВО (LEFT) в IDLE запустит перезапись параметров текущего
  профиля на параметры встроенного тестового профиля.
  20. Короткое нажатие кнопки ВПРАВО (RIGHT) в IDLE включит внешний вентилятор.
  21. Короткое нажатие кнопки ВПРАВО (RIGHT) внутри основного меню
  переключает на следующий параметр для редактирования.
  22. Короткое нажатие кнопки ВЛЕВО (LEFT) в IDLE выключит внешний вентилятор, если он
  перед этим был включен кнопкой ВПРАВО (RIGHT).
  23. Перед компиляцией скетча, закомментировав строку [ #define SetInterrupt 0 ],
  ...
  24. Любой профиль теперь можно использовать для реализации управления термостолом,
  ...
  до момента нажатия кнопки ВЛЕВО (LEFT). Чтобы после старта профиля управления термостолом
  начался процесс роста температуры, нужно его принудительно вручную активировать
  кнопкой ВПРАВО (RIGHT), т.к. по умолчанию, при управлении верхним излучателем, старт
  ...
  25. После первой загрузки скетча в плату arduino, настоятеьно рекомендуется произвести
  ...
  26. /* //if (counter >= profile.dwellTimerStep[currentStep - 1]) { //если счётчик больше установленного времени
